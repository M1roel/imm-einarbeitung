# Lernmodulentwurf: Neues Leihset aus einer vorhandenen Schablone in Instacount anlegen

## Kennzeichnung und Verwendung

- **Modulstatus:** `Entwurf – in fachlicher Prüfung`
- **Fachlicher Arbeitsstand:** `fachliche Auskunft 25.09.2026` eines erfahrenen IMM-Mitarbeiters/Stammdatenmanagers; als standortübergreifender Standard vorgesehen, aber noch nicht formal standortübergreifend freigegeben.
- **Historische Quellenbasis:** Aussagen mit dem Status `quellenbelegt` sind an den angegebenen Stellen in `Anlegen Set_Leih_überarbeitet.pdf` beziehungsweise im Einarbeitungsplan belegt.
- **Didaktische Gestaltung:** `Plattformvorschlag`.
- **Formale Freigabe:** nicht vorhanden. Die fachliche Auskunft wird nicht mit einer formalen standortübergreifenden Freigabe gleichgesetzt.
- **Verwendung:** Nur als gekennzeichneten Lernentwurf verwenden und zur formalen Freigabe vorlegen; keine unbegleitete Anwendung als verbindliche Arbeitsanweisung.

## 1. Einordnung in den Lernpfad

- **Lernpfad:** „Management von Leihinstrumenten“ — `quellenbelegt` durch `F_05-04-01-09_Einarbeitungsplan IMM_2021-10-18.pdf`, Abschnitt 6, PDF-Seite 4.
- **Modulthema:** Ein neues Leihset aus einer vorhandenen Schablone anlegen und die unmittelbar folgenden Kontrollen und Dokumentationsschritte ausführen — `fachliche Auskunft 25.09.2026`; noch nicht formal standortübergreifend freigegeben.
- **Softwareumfang:** ausschließlich Instacount 4.42 in der ersten Ausbaustufe — `fachliche Auskunft 25.09.2026`; die PDF zeigt Instacount PLUS, belegt aber die Version 4.42 nicht.
- **Vorgesehener Geltungsbereich:** ein einheitlicher Ablauf für alle Standorte — `fachliche Auskunft 25.09.2026`; formale standortübergreifende Freigabe offen.

## 2. Zielgruppe

- IMM-Beschäftigte mit der für den Ablauf erforderlichen Instacount-Berechtigung — `Plattformvorschlag` auf Basis der fachlichen Auskunft.
- Lernende, die Herstellerbewertung und Risikoeinstufung nicht in diesem Modul vornehmen, sondern mit bereits geprüften Ergebnissen weiterarbeiten — `Plattformvorschlag`.

## 3. Konkretes Lernziel

Nach Bearbeitung kann die lernende Person in einer Lern- oder begleiteten Praxisumgebung:

- vorgegebene Voraussetzungen prüfen und bei einem fehlenden Pflichtbestandteil stoppen,
- einen Leihset-Code für einen synthetischen Fall nachvollziehbar zusammensetzen,
- ein Set aus einer passenden vorhandenen Schablone in der für Instacount 4.42 vorgesehenen Abfolge erzeugen,
- geerbte und setspezifische Dokumente unterscheiden,
- die Etikettenmenge berechnen,
- die wiederkehrende Setfotografie von einem einmaligen Stammdatenbild abgrenzen,
- den gesonderten Ausnahmeweg bei fehlendem Lieferschein erkennen, ohne weitere Ausnahmen zu erfinden.

Das Lernziel ist ein `Plattformvorschlag` auf Basis der `fachlichen Auskunft 25.09.2026` und der jeweils genannten PDF-Fundstellen.
...
Dieses Modul umfasst nicht:

- Bewertung der Herstellerangaben — `Plattformvorschlag`; als eigener Tätigkeitsbereich historisch `quellenbelegt` im Einarbeitungsplan, Abschnitt 6, PDF-Seite 4.
- eigenständige Risikoeinstufung oder Änderung einer Einstufung — `Plattformvorschlag`; eine vorhandene Klassifizierung wird in der Anleitung auf PDF-Seiten 19 und 21 vorausgesetzt.
- SPMi — `Plattformvorschlag`; die erste Ausbaustufe ist laut fachlicher Auskunft auf Instacount 4.42 begrenzt.
- Erstellung einer neuen Schablone — `Plattformvorschlag`; verwendet wird eine vorhandene passende Schablone.
- zusätzliche Ausnahmewege — `fachliche Auskunft 25.09.2026`; außer dem fehlenden Lieferschein wurde kein weiterer Ausnahmefall bestätigt.

## 6. Eigener Lernabschnitt: Setcode bilden

### 6.1 Aufbau

`fachliche Auskunft 25.09.2026` — Eine Leihset-Schablone wird am Bestandteil `-L-` erkannt. Der Setcode folgt dem vorgesehenen Muster:

`<MANDANT>-<FACHBEREICH>-<EINSATZBEREICH>-L-<LAUFENDE NUMMER>`

Die PDF belegt historisch den hierarchischen Aufbau und `-L-` als Leihset-Merkmal auf PDF-Seiten 1, 6–9 und 17. Die aktuelle Segmentbezeichnung und Auslegung stammen aus der fachlichen Auskunft und sind noch nicht formal standortübergreifend freigegeben.
...
Alle Schritte bilden den vorgesehenen Arbeitsstand ab, sind aber noch nicht formal standortübergreifend freigegeben.
...
| 6 | Kontrolliere, dass Herstellerangaben und Bewertung aus der Schablone übernommen wurden. Ergänze am erzeugten Set Anmeldung, Lieferschein und Bilder. | `fachliche Auskunft 25.09.2026` und historisch `quellenbelegt` auf PDF-Seiten 23, 27 und 28 | `Plattformvorschlag`: Zwei-Spalten-Vergleich „aus Schablone“/„am Set ergänzen“ |
| 7 | Führe nach der Seterzeugung Setfotografie, PDF-Zuordnung von Anmeldung und Lieferschein, abschließende Kontrolle und Etikettendruck aus. | `fachliche Auskunft 25.09.2026`; Dokumentzuordnung/Etikett/Fotografie historisch `quellenbelegt` auf PDF-Seiten 28–37 und 39–50; Abschlusskontrolle nicht ausreichend in der PDF beschrieben | `Plattformvorschlag`: Abschluss-Checkliste mit getrennten Nachweisen |
| 8 | Bei Rückgabe an den Verleiher löse das Set von der Schablone und setze es auf `Gesperrt`. | `fachliche Auskunft 25.09.2026` und historisch `quellenbelegt` auf PDF-Seiten 20 und 38 | `Plattformvorschlag`: eigener Lebenszyklus-Hinweis; nicht als Teil der Erstanlage darstellen |

## 8. Gesonderter Ausnahmeweg: Lieferschein fehlt
...
Die folgende Regel ist `fachliche Auskunft 25.09.2026`, noch nicht formal standortübergreifend freigegeben:

- `A = 1` Etikett für die Anmeldung,
- `C = 1` Etikett für den Containeranhänger, falls ein Container vorhanden ist, sonst `0`,
- `S = Anzahl der Siebschalen`, je ein Etikett für deren Kofferanhänger.

`Etiketten gesamt = 1 + C + S`

`Plattformvorschlag` — Die Berechnung wird im Interface mit Zwischensumme angezeigt.

Synthetische Beispiele:

- Container vorhanden, drei Siebschalen: `1 + 1 + 3 = 5` Etiketten.
- Kein Container, zwei Siebschalen: `1 + 0 + 2 = 3` Etiketten.

Die PDF belegt auf PDF-Seiten 36–37 Etikettdruck und ein Etikett für die Anmeldung, nicht jedoch die vollständige Berechnungsregel.
...
### Frage 1: Setcode zusammensetzen
...
- **Status:** `Plattformvorschlag`; erwartete Antwort basiert auf `fachlicher Auskunft 25.09.2026`.
- **Fragetyp:** Rechenfrage mit Einfachauswahl
- **Frage:** Ein synthetisches Leihset hat einen Container und drei Siebschalen. Wie viele Etiketten sind nach der mitgeteilten Regel vorgesehen?
- **Antwortmöglichkeiten:**
  - A: 3
  - B: 4
  - C: 5
  - D: 6
- **Richtige Antwort:** C
- **Begründung:** Anmeldung `1` + Containeranhänger `1` + drei Kofferanhänger `3` = `5`.
- **Quellenbezug:** Fachliche Klärung vom 25.09.2026, Punkt 15; PDF-Seiten 36–37 belegen nur Teile der Regel.

### Frage 4: Zweck und Häufigkeit der Setfotografie

- **Status:** `Plattformvorschlag`; erwartete Antwort basiert auf `fachlicher Auskunft 25.09.2026` und ist teilweise `quellenbelegt`.
- **Fragetyp:** Mehrfachauswahl
- **Frage:** Welche Aussagen beschreiben die vorgesehene Setfotografie richtig?
- **Antwortmöglichkeiten:**
...
Alle Daten und Bezeichnungen in der Übung sind synthetisch. Die Übung ist ein `Plattformvorschlag`.

### Teil A: regulärer Weg

Für die Lernumgebung liegen vor:
...
- bestätigte Vollständigkeit,
- ein Container und drei Siebschalen.
...
| Metadatum | Sichtbarer Wert | Status |
| --- | --- | --- |
| Modulstatus | `Entwurf – in fachlicher Prüfung` | `Plattformvorschlag` |
| Fachliche Auskunft | Erfahrener IMM-Mitarbeiter/Stammdatenmanager, 25.09.2026 | fachlicher Arbeitsstand; noch nicht formal standortübergreifend freigegeben |
| Geltungsabsicht | Einheitlicher Ablauf für alle Standorte | `fachliche Auskunft 25.09.2026`; formale Freigabe offen |
| Software | Instacount 4.42, erste Ausbaustufe | `fachliche Auskunft 25.09.2026` |
| Fachliche Klärung | `docs/fachliche-klaerungen/leihset-ablauf-2026-09-25.md` | dokumentierte Auskunft und Quellenabgleich |
...
Diese Fragen bleiben trotz der fachlichen Auskunft offen und müssen vor formaler standortübergreifender Freigabe entschieden werden:
...
Alle Bausteine sind `Plattformvorschlag`; es wird in diesem Auftrag kein Angular-Code erstellt.
